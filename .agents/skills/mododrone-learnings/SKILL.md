...
- **2026-08-06**:
  - **Hover Preview estilo YouTube Limpio & Resaltado de Tarjeta Sin Borde**: Perfeccionamiento de la vista previa al hacer hover (`onmouseenter`) agregando `scale-[1.08]` y parámetros `controls=0&iv_load_policy=3&modestbranding=1&autohide=1` para eliminar elementos del reproductor nativo. Remoción del borde en hover (`hover:border-transparent`), conservando únicamente el resaltado suave de fondo (`hover:bg-white/[0.08]`). Remoción limpia e inmediata al retirar el cursor (`onmouseleave`).
