...
- **2026-08-06**:
  - **Hover Preview estilo YouTube Limpio & Resaltado de Tarjeta**: Perfeccionamiento de la vista previa al hacer hover (`onmouseenter`) agregando `scale-[1.08]` y parámetros `controls=0&iv_load_policy=3&modestbranding=1&autohide=1` para eliminar elementos/bordes del reproductor nativo. Incorporación del cambio sutil de fondo y borde iluminado de la tarjeta contenedora al hacer hover (`hover:bg-white/[0.08] hover:border-white/20`). Remoción limpia al retirar el cursor (`onmouseleave`).
