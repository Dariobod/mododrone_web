...
- **2026-08-06**:
  - **Hover Preview estilo YouTube Limpio & Tarjetas Sin Borde**: Perfeccionamiento de la vista previa al hacer hover (`onmouseenter`) agregando `scale-[1.08]` y parámetros `controls=0&iv_load_policy=3&modestbranding=1&autohide=1` para eliminar elementos del reproductor nativo. Eliminación completa de bordes en las tarjetas (`!border-none`), dejando únicamente el cambio sutil de color de fondo al hacer hover (`hover:bg-white/[0.08]`). Remoción limpia e inmediata al retirar el cursor (`onmouseleave`).
